...
mapLoop:
	lw	x6 0(x8)	# load the address of the array of current node into t1
	lw	x7 4(x8)		# load the size of the node's array into t2
    mul x29, x5, x28

	add	x6, x6, x29		# offset the array address by the count
	...
	blt	x5, x7, mapLoop	# repeat if we haven't reached the array size yet

	lw	x10, 8(x8)		# load the address of the next node into a0
	add	x11,x9,x0	# put the address of the function back into a1 to prepare for the recursion
